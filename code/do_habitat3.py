...
savepath='/root/work/BIO/new_code/res/ani_byBW'
...
def get_habitat(ID,savepath,type='ALL',year=1990):
    ...
    info=info[info['category']==type]

    uinfo=getinfo(LC)
    res=np.zeros((uinfo['row'],uinfo['col']))
    
    for i,item in info.iterrows():
        print(f'\r doing-{type}-{ID}-{i}/{info.shape[0]}',end='',flush=True)
        name=item['name']
        ani_shp=animal_shps[item['type']]
        cho=ani_shp[ani_shp['binomial' if item['type']!='BIRD' else 'sci_name']==name]
        ...
        ds=gdal.Rasterize(
                midtifpath,
                f'/root/work/BIO/new_code/data/china_{item["type"]}_lam.shp',
                format='GTiff',
                ...
                # width=uinfo['col'],
                # height=uinfo['row'],
                SQLStatement=f"SELECT * from china_{item['type']}_lam WHERE id_no={id_no}",
                # attribute=field,
                burnValues=1,
                initValues=0,
                )
...
if __name__=="__main__":
    ...
    # for anitype in anitypes:
    ty=sys.argv[1]
    apath=os.path.join(savepath,ty)
    if os.path.exists(apath):
        shutil.rmtree(apath)
    os.mkdir(apath)
    for ID in patchs['ID']:
        get_habitat(ID,os.path.join(apath,f'Habitat_{ID}.tif'),type=ty)
        # print(f'\r doing-{ty}-{ID}',end='',flush=True)
