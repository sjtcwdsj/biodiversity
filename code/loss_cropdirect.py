...
haibitat_path='/root/work/BIO/new_code/res/ani_byBW'
...
def getlossall(ID,anitype,savepath):
    starttime=time.time()
    habitatopath=os.path.join(haibitat_path,anitype,f'Habitat_{ID}.tif')
    ...
    for year in range(1990,2020):
        ...
        endtime=time.time()
        print(f'\r{anitype}-{ID}-{year}-{(endtime-starttime):.2f}',end='',flush=True)
    ...
    respd.to_csv(os.path.join(savepath,f'{ID}_td.csv'),index=0)
...
if __name__=="__main__":
    savepath='/root/work/BIO/fix/res/loss/loss_cropdirect'
